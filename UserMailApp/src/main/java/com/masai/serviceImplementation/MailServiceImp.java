...
import com.masai.serviceInerface.MailService;
...
public class MailServiceImp implements MailService {
	...
	public Mail sendMail(Mail mail) throws MailException, UserException {
		...
		List<CurrentUserSession> lin=lr.findAll();
		if(lin.size()>0) {
			for(CurrentUserSession l:lin) {
				
				Optional<User> u1=ur.findById(l.getUserId());	
				Optional<User> u2=ur.findById(mail.);
			...
			throw new MailException("error");	// edit
		}else {
